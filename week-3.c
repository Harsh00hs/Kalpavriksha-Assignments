...
// str2 going to str1
void stringCopier(char *str1, char *str2){
    int i=0;
    while(str2[i] != '\0'){
        str1[i]=str2[i];
        i++;
...
int main(){
    ...
    scanf("%d", &r);
    while(r<1 || r>10){
        printf("Invalid: So enter between 1-10\n");
        scanf("%d", &r);
    }
    ...
    scanf("%d", &c);
    while(c<1 || c>101){
        printf("Invalid: So enter between 1-101\n");
        scanf("%d", &c);
    }
    ...
    for(int i=0; i<r; i++){
        for(int j=0; j<c; j++){
            ...
            if(stringLength(student[i][j]) > stringLength(longestName))
                stringCopier(longestName, student[i][j]);
        }
    }

    printf("\n2d Array Name of Students: \n");
    for(int i=0; i<r; i++){
        for(int j=0; j<c; j++){
